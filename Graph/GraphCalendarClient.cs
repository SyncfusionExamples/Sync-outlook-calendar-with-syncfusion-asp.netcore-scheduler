...
namespace DotNetCoreRazor_MSGraph.Graph
{
    public class GraphCalendarClient
    {
        ...
        public async Task<IEnumerable<Event>> GetEvents(string userTimeZone, DateTime start, DateTime end)
        {
            ...
            try
            {
                ...
                var calendarEvents = await _graphServiceClient.Me.CalendarView.Request(viewOptions).Header("Prefer", $"outlook.timezone=\"{userTimeZone}\"")
                    .Select(evt => new { evt.Subject, evt.Organizer, evt.Start, evt.End }).OrderBy("start/DateTime").GetAsync();
